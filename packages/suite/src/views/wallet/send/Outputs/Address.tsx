    ...
    const { ref: inputRef, ...inputField } = register(inputName, {
        ...
        validate: {
            ...
            evmChecks: async (checkedAddress: string) => {
                ...
                if (isNamedInput) {
                    // The form validates on every keystroke, so resolving straight away fires a
                    // request per intermediate value ("cult.et" on the way to "cult.eth"). Wait
                    // out the burst, then bail if the field moved on: the newer run starts its
                    // own wait later, so it is always the one that settles last and decides.
